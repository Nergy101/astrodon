...
export async function build(options: BuildOptions): Promise<void> {
    ...
    const resolvedAssetsDir = options.assetsDir ?? new URL('assets', pkgBase).pathname;
    const resolvedLuaDir = options.luaDir ?? new URL('lua-scripts', pkgBase).pathname;
    const resolvedTemplate = options.template ?? new URL('template.lua', pkgBase).pathname;
